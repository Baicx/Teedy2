...
RUN mkdir /app && \
cd /app && \
java -jar /opt/jetty/start.jar --add-modules=server,http,webapp,deploy,jsp,annotations
# Add the local file docs.xml and the built WAR file docs-web-*.war to the container's Jetty web applications directory.Allows Jetty to load these webapplications at startup.
